...
void highlight(list<string>&lststr,list<string>&lstcolor,list<string>::iterator changed,list<string>::iterator clr,int cval)
{
    ...
    string currstr=*changed,currcolor=*clr;
    int startpoint=-1;
    ...
    while(1)
    {
        ...
        if(status==IDENTIFIER)
        {
            ...
            cout << startid;
            char id_color = 'i';
            constexpr const char *type_list[] = {"bool","short","int","long","float","double",
                "class","enum","struct","union","signed","unsigned","typedef","const",
                "volatile","static","extern","constexpr","void"
            };
            constexpr const char *control_list[] = {"break","continue","for","if","while",
                "do","switch","case","default","try","catch","throw"
            };
            for(const char * const & current_type:type_list)
                if(id == current_type)
                    id_color = 't';
            if(id_color == 'i')
                for(const char * const & current_type:control_list)
                    if(id == current_type)
                        id_color = 'c';
            for(int i = 0; i < idlength; ++i)
                currcolor[startid + i] = id_color;
        }
        if(status==WAITING)
        {
            currcolor[currpoint]='-';
